...
function GetAccountAddresses(account) {
	coin.api.exec('getaddressesbyaccount', account, function(err, res) {
		//console.log("DEBUG: Account '" + account + "' node_id: " + coin.rpcHost);
		if (typeof res !== 'undefined') {
			...
			if (account === coin.settings.masterAccount) {
				...
				doneGetAccountAddresses = 0; // set global flag-counter to break out of interval
				// Sweep for empty account labels.
				if (accountAddresses && accountAddresses.length) {
			...
			console.log("Error: No addresses for account: " + account + " node_id: " + coin.rpcHost);
			console.log(err);
		}
	});
}
...
function Init() {
	...
	User.findOne({'local.id': coin.settings.masterAccount, 'wallet.node_id': coin.rpcHost}, function(err, user) {
		if (user) {
			...
			console.log("Init-Wallet: Found " + coin.settings.masterAccount + "'s wallet for the " + coin.settings.coinName + " node_id on: " + coin.rpcHost);
