...
module.exports = function(passport) {
	...
	function(req, email, password, done){
		...
		coin.api.exec('getaccountaddress', account, function(err, res){
			address = res;
			if (address === ""){
		...
		process.nextTick(function(){
			...
			User.findOne({'local.id': email}, function(err, user){
				...
				if(user){
					...
					if (new_address){
						user.wallet.push( { node_id: coin.rpcHost, account: account, address: address });
						// sendfrom <fromaccount> <toaddress> <amount> [minconf=1] [comment] [comment-to] [txcomment]
						coin.api.exec('sendfrom', coin.settings.masterAccount, address, coin.settings.newUserAmount, 1, "New Account", address, "Welcome to " + coin.settings.coinName + "!", function(err, res){
					...
					newUser.wallet.push( { node_id: coin.rpcHost, account: account, address: address });

					newUser.save(function(err){
						if(err)
							throw err;
						// sendfrom <fromaccount> <toaddress> <amount> [minconf=1] [comment] [comment-to] [txcomment]
						coin.api.exec('sendfrom', coin.settings.masterAccount, address, coin.settings.newUserAmount, 1, "New Account", address, "Welcome to " + coin.settings.coinName + "!",
							function(err, res){
								if (err) console.log("Error: err:" + err + " res:" + res);
							}
						);
