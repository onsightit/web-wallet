	...
	function(req, email, password, done) {
		...
		process.nextTick(function() {
			User.findOne({'local.id': email}, function(err, user) {
				if (err)
					return done(err);
				// TODO: If user exists, redirect to login page immediately and do not create a new address!!!
				if (user) {
					...
					if (new_address) {
						...
						coin.api.exec('sendfrom', coin.settings.masterAccount, addresses[0], coin.settings.newUserAmount, 1, "New Account", addresses[0], "Welcome to " + coin.settings.appTitle + "!", function(err, res) {
		...
		function(req, email, password, done) {
			...
			process.nextTick(function() {
				User.findOne({ 'local.id': email}, function(err, user) {
					...
					setTimeout(function() {
						if (new_address && addresses.length) {
							// sendfrom <fromaccount> <toaddress> <amount> [minconf=1] [comment] [comment-to] [txcomment]
							coin.api.exec('sendfrom', coin.settings.masterAccount, addresses[0], coin.settings.newUserAmount, 1, "New Account", addresses[0], "Welcome to " + coin.settings.appTitle + "!", function(err, res) {
								if (err) console.log("Error: err:" + err + " res:" + res);
								});
	    			...
	    			if (user) {
	    				...
	    				newUser.save(function(err) {
	    					if (err)
	    						throw err;
							// sendfrom <fromaccount> <toaddress> <amount> [minconf=1] [comment] [comment-to] [txcomment]
							coin.api.exec('sendfrom', coin.settings.masterAccount, addresses[0], coin.settings.newUserAmount, 1, "New Account", addresses[0], "Welcome to " + coin.settings.appTitle + "!",
								function(err, res) {
									if (err) console.log("Error: err:" + err + " res:" + res);
								}
							);
